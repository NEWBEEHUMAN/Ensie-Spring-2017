...
class DefinitionRejectedNotificationListener implements EventSubscriberInterface{
    ...
    {
        return array(
            DefinitionEvents::DEFINITION_REJECTED => array(array('addNotification')),
        );
    }

    public function addNotification(DefinitionValidationEvent $event)
    {
        $this->notificationService->createNotification(
            $event->getDefinitionValidation()->getDefinition()->getEnsieUser(),
            NotificationTemplate::IDENTIFIER_REJECTED,
            array('definition' => $event->getDefinitionValidation()->getDefinition(), 'feedback' => $event->getDefinitionValidation()->getFeedback())
        );
